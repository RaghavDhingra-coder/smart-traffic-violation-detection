    ...
    <article className="rounded-3xl border border-white/70 bg-white p-5 shadow-panel">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div className="space-y-2">
          ...
          {challan.amount ? <p className="text-sm text-slate-600">Amount: <span className="font-semibold text-ember">?{challan.amount}</span></p> : null}
        ...
        >
          {isPaid ? "Paid" : loading ? "Opening gateway..." : "Pay Now"}
        </button>
      </div>
    </article>
  );
